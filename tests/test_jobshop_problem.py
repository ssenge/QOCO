...
from qoco.optimizers.rl.training.milp.selectors import PrefixBinarySelector, SelectorFactory


def _jobshop_instance() -> JobShopScheduling:
    jobs = [
        [Operation(machine=0, duration=1.0), Operation(machine=1, duration=1.0)],
        [Operation(machine=1, duration=1.0), Operation(machine=0, duration=1.0)],
    ]
    return JobShopScheduling(name="jobshop_2x2", jobs=jobs)
...
def _make_adapter(problem: JobShopScheduling):
    build_model = JobShopScheduling.MILPConverter().convert
    model = build_model(problem)
    kernel = compile_numeric_milp_kernel(model=model)
    selector_factory = SelectorFactory(
        make=lambda m, k: PrefixBinarySelector.from_model(m, k, prefix="y[")
    )
    selector = selector_factory.build(model, kernel).prepare(model, kernel)
    n_nodes = int(selector.n_actions)
    config = GenericMILPConfig(max_steps=max(1, n_nodes))
...
def test_jobshop_relex_rl4co() -> None:
